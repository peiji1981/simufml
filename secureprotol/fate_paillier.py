...
import numpy as np
from simufml.secureprotol.fixedpoint import FixedPointNumber
from simufml.secureprotol import gmpy_math
import random
...
class PaillierPublicKey(object):
    ...
    def encrypt(self, value, precision=None, random_value=None):
        ...
        def _encrypt(v):
            if (
                ...
                or 
                (isinstance(v, np.ndarray) and len(v.shape)==0)
            ):
                return self._encrypt_single(v, precision, random_value)
            else:
                res = []
                for e in v:
                    res.append(self.encrypt(e, precision, random_value))
                return res
    ...
    # xty modified
    def _encrypt_single(self, value, precision, random_value):
        """
        Encode and Paillier encrypt a real number value.
        """
        # transform 0-dim np.ndarray to scalar
        value = value.dtype.type(value) if isinstance(value, np.ndarray) else value
...
class PaillierPrivateKey(object):
    ...
    def decrypt(self, encrypted_value):
        ...
        def _decrypt(en_v):
            if (
                (not isinstance(en_v, Iterable)) 
                or 
                (isinstance(en_v, np.ndarray) and len(en_v.shape)==0)
            ):
                return self._decrypt_single(en_v)
            else:
                res = []
                for e in en_v:
                    res.append(_decrypt(e))
                return res

        res = _decrypt(encrypted_value)
        if isinstance(encrypted_value, np.ndarray):
            return np.array(res)
        else:
            return res
    ...
    # xty modified
    def _decrypt_single(self, encrypted_number):
